...
def login(request):
    # si el usuario esta logeado no puede volver a logearse
...
def registration(request):
    #si el usuario esta logeado no puede entrar a la pagina de registro
    if request.session.has_key('user_id'):
        return redirect(reverse('app:index'))
    if request.method == 'POST':
...
def check_login(request):
    ...
    result = conn.cursor()
    result.execute("SELECT USER_ID, USER_NAME,PASSWORD FROM APP.CUSTOMER WHERE USER_NAME= :mybv",
                   mybv=username)
    cnt = result.fetchone()
    # username 
    if cnt is None:
...
def login_admin(request):
    # si el usuario esta logeado no puede visitar la pagina de logeo
    if request.session.has_key('user_id'):
...
def registration_admin(request):
    # solo un administrador puede registrar a otro administrador
